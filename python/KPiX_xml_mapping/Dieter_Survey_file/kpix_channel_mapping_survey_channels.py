...
filename_tracker_to_kpix = "tracker_to_kpix_right.txt"
f = open(filename_tracker_to_kpix, "r")#open the tracker_to_kpix (left/right) file 

filename_noisy_channels = "survey.txt"
...
filename_map = 'map_survey_and_DC_deactivated.txt'		#choose filename to save as
	
for char in range(1024):			#create the character mapping of the channels
	if char in noisy_channels or char in dc_channels:		#D for noisy channels
		kpix[char] = 'D'
...
#save it in filename with a format that fits in the xml file, it adds the space after 8 characters so it's easier to see (but a pain to write it like this, may be another way) 
