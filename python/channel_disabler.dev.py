...
class ChannelDisabler:
    ...
    def run(self):
        ...
        for chan in xrange(1024):		#create the character mapping of the channels
            ...
            if ( (chan in dc_channels) and not self.ecal) or (chan in dead_channels_slope) or (chan in dead_channels_RMS) or (chan in noisy_channels) or (chan in noisy_channels_acquire):		#A for active and D for deactivated channels
		kpix[chan] = 'D'
	    else :
		kpix[chan] = 'A'
